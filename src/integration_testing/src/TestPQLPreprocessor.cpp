...
//	PQLParser parser = PQLParser();
//	queue<QueryToken> qq = parser.parse("while w1, w2; procedure p1, p2, p3;");
//	parser.buildQuery(qq);
//	Query q = parser.getQuery();
...
SCENARIO("Including one such that clause") {
	PQLParser parser = PQLParser();
	queue<QueryToken> qq = parser.parse("variable a1 , a2; if ifs; Select ifs such that Follows(ifs ,12)");
	parser.buildQuery(qq);
	Query q = parser.getQuery();
