...
TEST_CASE("Testing while a, follows a unexpected keyword")
{
	...
	SECTION("1") {
		...
		REQUIRE_THROWS_WITH(PQLLexer(input), "should follows a new declaration or selection");
		//REQUIRE_THROWS_WITH(p.getTokenQueue(), "no selction after the declaration.");
	}
}
...
TEST_CASE("Testing if a, select a with semicolumn")
{
	...
	SECTION("1") {
		REQUIRE_THROWS_WITH(PQLLexer(input), "no semicollumn");
...
TEST_CASE("Testing pattern following a unexpected string")
{
	...
	SECTION("1") {
		REQUIRE_THROWS_WITH(PQLLexer(input), "should be pattern or such that or and");
...
{
	const string input = "assign a; stmt s; variable v; Select a such that Modifies (\"x\", \"x\")";
	queue<pair<TokenType, string>> res;
	PQLLexer p(input);
...
{
	const string input = "assign a; variable v; Select a such that Uses \"x\", v);";
...
{
	const string input = "assign a; variable v; Select a such that Modifies \"x\", v);";
...
{
	const string input = "assign a; variable v; Select a such that Modifies (a, v)";
