...
TEST_CASE(
    "One complex nesting in one procedure in program with two procedures") {
  std::unique_ptr<PKB> pkb{new PKB()};
  pkb->setProc("A", 1, 4);
  pkb->setProc("B", 4, 23);
  pkb->setNext(1, 2);
...
TEST_CASE("Affects with no if/while stmt") {
  std::unique_ptr<PKB> pkb{new PKB()};
  pkb->setProc("A", 1, 4);
  pkb->setNext(1, 2);
  pkb->setNext(2, 3);
...
TEST_CASE("Affects with if stmt") {
  std::unique_ptr<PKB> pkb{new PKB()};
  pkb->setProc("A", 1, 6);
  pkb->setNext(1, 2);
  pkb->setNext(2, 3);
...
TEST_CASE("Affects with while stmt with x=x") {
  std::unique_ptr<PKB> pkb{new PKB()};
  pkb->setProc("A", 1, 6);
  pkb->setNext(1, 2);
  pkb->setNext(2, 3);
...
TEST_CASE("AffectsT with read stmt and no nesting") {
  std::unique_ptr<PKB> pkb{new PKB()};
  pkb->setProc("A", 1, 4);
  pkb->setNext(1, 2);
  pkb->setNext(2, 3);
...
TEST_CASE("AffectsT with call stmt and no nesting") {
  std::unique_ptr<PKB> pkb{new PKB()};
  pkb->setProc("A", 1, 4);
  pkb->setProc("B", 4, 5);
  pkb->setCalls("A", "B");
...
TEST_CASE("AffectsT with one if") {
  std::unique_ptr<PKB> pkb{new PKB()};
  pkb->setProc("A", 1, 5);
  pkb->setNext(1, 2);
  pkb->setNext(1, 3);
