...
TEST_CASE("Populate CFG") {
  ...
  SECTION("CFG with one if and one while statement") {
    ...
    REQUIRE_THAT(forwardCFG[0], Contains({1, 2}));
    REQUIRE_THAT(forwardCFG[1], Contains({0}));
    REQUIRE_THAT(forwardCFG[2], Contains({3, 4}));
    REQUIRE_THAT(forwardCFG[3], Contains({5}));
    REQUIRE_THAT(forwardCFG[4], Contains({5}));
    REQUIRE_THAT(forwardCFG[5], Contains(empty));
    REQUIRE_THAT(reverseCFG[0], Contains({1}));
    REQUIRE_THAT(reverseCFG[1], Contains({0}));
    REQUIRE_THAT(reverseCFG[2], Contains({0}));
    REQUIRE_THAT(reverseCFG[3], Contains({2}));
    REQUIRE_THAT(reverseCFG[4], Contains({2}));
    REQUIRE_THAT(reverseCFG[5], Contains({3, 4}));
  }
  ...
  SECTION("CFG with two if statements nested") {
    ...
    REQUIRE_THAT(forwardCFG[0], Contains({1}));
    REQUIRE_THAT(forwardCFG[1], Contains({2, 3}));
    REQUIRE_THAT(forwardCFG[2], Contains({4, 5}));
    REQUIRE_THAT(forwardCFG[3], Contains({6}));
    REQUIRE_THAT(forwardCFG[4], Contains({6}));
    REQUIRE_THAT(forwardCFG[5], Contains({6}));
    REQUIRE_THAT(forwardCFG[6], Contains(empty));
    REQUIRE_THAT(reverseCFG[0], Contains(empty));
    REQUIRE_THAT(reverseCFG[1], Contains({0}));
    REQUIRE_THAT(reverseCFG[2], Contains({1}));
    REQUIRE_THAT(reverseCFG[3], Contains({1}));
    REQUIRE_THAT(reverseCFG[4], Contains({2}));
    REQUIRE_THAT(reverseCFG[5], Contains({2}));
    REQUIRE_THAT(reverseCFG[6], Contains({3, 4, 5}));
  }
  ...
  SECTION("CFG with two while statements nested") {
    ...
    REQUIRE_THAT(forwardCFG[0], Contains({1, 2}));
    REQUIRE_THAT(forwardCFG[1], Contains({0, 3}));
    REQUIRE_THAT(forwardCFG[2], Contains(empty));
    REQUIRE_THAT(forwardCFG[3], Contains({1}));
    REQUIRE_THAT(reverseCFG[0], Contains({1}));
    REQUIRE_THAT(reverseCFG[1], Contains({0, 3}));
    REQUIRE_THAT(reverseCFG[2], Contains({0}));
    REQUIRE_THAT(reverseCFG[3], Contains({2}));
  }
  ...
  SECTION("CFG with while nested in if statement") {
    ...
    REQUIRE_THAT(forwardCFG[0], Contains({1, 2}));
    REQUIRE_THAT(forwardCFG[1], Contains({4}));
    REQUIRE_THAT(forwardCFG[2], Contains({3, 4}));
    REQUIRE_THAT(forwardCFG[3], Contains({2}));
    REQUIRE_THAT(forwardCFG[4], Contains(empty));
    REQUIRE_THAT(reverseCFG[0], Contains(empty));
    REQUIRE_THAT(reverseCFG[1], Contains({0}));
    REQUIRE_THAT(reverseCFG[2], Contains({0, 3}));
    REQUIRE_THAT(reverseCFG[3], Contains({2}));
    REQUIRE_THAT(reverseCFG[4], Contains({1, 2}));
  }
  SECTION("CFG with if nested in while statement") {
    ...
    REQUIRE_THAT(forwardCFG[1], Contains({2, 3}));
    REQUIRE_THAT(forwardCFG[2], Contains({4, 5}));
    REQUIRE_THAT(forwardCFG[3], Contains(empty));
    REQUIRE_THAT(forwardCFG[4], Contains({3}));
    REQUIRE_THAT(forwardCFG[5], Contains({3}));
    REQUIRE_THAT(reverseCFG[0], Contains(empty));
    REQUIRE_THAT(reverseCFG[1], Contains({0, 4, 5}));
    REQUIRE_THAT(reverseCFG[2], Contains({3}));
    REQUIRE_THAT(reverseCFG[3], Contains({1}));
    REQUIRE_THAT(reverseCFG[4], Contains({2}));
    REQUIRE_THAT(reverseCFG[5], Contains({2}));
  }
