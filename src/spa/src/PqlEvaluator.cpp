...
ClauseResult PqlEvaluator::getModifies(Clause c) {
  ...
  if (!isSynonym(iter1->type)) {
	  set<string> usedBy = mypkb.getUses(iter1->name);
    if(!usedBy.empty())
    {
		set<string>::iterator iterr = usedBy.begin();
		vector<vector<string>> result;
		
      while(iterr!=usedBy.end())
      {
		  vector<string> resultPair;
		  resultPair.push_back(iter1->name);
		  resultPair.push_back(*iterr);
