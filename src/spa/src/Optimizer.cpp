...
vector<string> Optimizer::joinSchema(vector<string> left, vector<string> right) {
...
int PlanCost::calculateCost(node* root) {
	if (root->isJoin) {
		...
		vector<string> schema = Optimizer::joinSchema(root->left->schema, root->right->schema);
		int diff =(root->left->schema.size() + root->right->schema.size()) - schema.size();
		if (diff == 0) {
